...
let template = [
  {
    ...
    submenu: [
      {
        // 以下是用 webContents.send是因为这是当前页面内的监听操作
        label: "新建",
  ...
  },
  {
    ...
    submenu: [
      ...
      },
      {
        ...
        click: () => {
          let isAutoSync = settingsStore.get("enableAutoSync");

          settingsStore.set("enableAutoSync", !isAutoSync);

          // ipcMain.emit("autosync-is-changed", !isAutoSync);
        },
      },
      {
        label: "全部同步至云端",
      ...
      },
      {
        ...
        enabled: qiniuIsConfiged,
        click: () => {
          ipcMain.emit("download-all-files-from-qiniu");
        },
      },
    ],
  },
